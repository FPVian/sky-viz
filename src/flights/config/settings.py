...
from flights.config.groups.db import Db, Postgres, Sqlite
...
# test running hydra inside a python package: pip .

# add a suppress_errors option to "general" config
# make a config for streamlit


# remove need for database driver setting...instantiate db repo in configs (remove from alembic env.py)
# resolve differences between database settings
...
def register_config_options():
    ...
    cs.store(group='db', name='postgres', node=Postgres)
